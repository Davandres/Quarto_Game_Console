...
from src.utils.display import display_boards
from colorama import Fore, Back, Style
import os

@click.command()
@click.option('--matches', default=1, help='Número de partidas a jugar', type=int)
@click.option('--player1', default='ai1', 
              type=click.Choice(['ai1', 'ai2'], case_sensitive=False),
              help='Tipo de jugador 1 (ai1 o ai2)')
@click.option('--player2', default='ai2', 
              type=click.Choice(['ai1', 'ai2'], case_sensitive=False),
              help='Tipo de jugador 2 (ai1 o ai2)')
@click.option('--delay', default=1.0, help='Retardo entre movimientos en segundos', type=float)
...
def play_quarto(matches, player1, player2, delay, verbose):
    ...
    results = {f"{player1.upper()} (P1)": 0, 
               f"{player2.upper()} (P2)": 0, 
               "Empates": 0}
